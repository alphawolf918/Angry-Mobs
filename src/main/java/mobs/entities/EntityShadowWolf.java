...
import angrymobs.mobs.entities.ai.EntitySWAIBeg;
import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;

public class EntityShadowWolf extends EntityTameable {
	...
	public EntityShadowWolf(World par1World) {
		super(par1World);
		...
		this.tasks.addTask(8, new EntitySWAIBeg(this, 8.0F));
		...
		this.setAlwaysRenderNameTag(this.hasCustomNameTag());
		this.setTamed(false);
	}
	
	@Override
	public void onDeath(DamageSource ds) {
		if (ds == DamageSource.inFire || ds == DamageSource.lava
				|| ds == DamageSource.onFire) {
			EntityGhostWolf gw = new EntityGhostWolf(worldObj);
			gw.setLocationAndAngles(this.posX, this.posY,
					this.posZ, this.rotationYaw, this.rotationPitch);
			if (this.hasCustomNameTag()) {
				gw.setCustomNameTag(this.getCustomNameTag());
			}
			this.worldObj.spawnEntityInWorld(gw);
			this.setDead();
		}
	}
	...
	protected void playStepSound(int par1, int par2, int par3, int par4) {
		this.playSound("mob.wolf.step", 0.20F, 2.0F);
	}
	...
	public boolean interact(EntityPlayer par1EntityPlayer) {
		...
		if (this.isTamed()) {
			...
			if (par1EntityPlayer.getCommandSenderName().equalsIgnoreCase(
				...
				&& itemstack.itemID == Items.ShadowBone.itemID) {
			if (!par1EntityPlayer.capabilities.isCreativeMode) {
				--itemstack.stackSize;
			}
	 ...
	 */
	public EntityShadowWolf spawnBabyAnimal(EntityAgeable par1EntityAgeable) {
		EntityShadowWolf entitywolf = new EntityShadowWolf(this.worldObj);
	...
	public boolean canMateWith(EntityAnimal par1EntityAnimal) {
		if (par1EntityAnimal == this) {
			...
			return false;
		} else if (!(par1EntityAnimal instanceof EntityShadowWolf)) {
			return false;
		} else {
			EntityShadowWolf entitywolf = (EntityShadowWolf) par1EntityAnimal;
	...
	public boolean func_142018_a(EntityLivingBase par1EntityLivingBase,
		...
		if (!(par1EntityLivingBase instanceof EntityCreeper)
			...
			if (par1EntityLivingBase instanceof EntityShadowWolf) {
